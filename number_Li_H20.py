...
output_file_path = os.path.join(current_folder, 'number_Li_H2O.csv')
...
with open(output_file_path, mode='a', newline='') as output_file:
    ...
    for folder in os.listdir(main_folder):
        ...
        if os.path.isdir(folder_path):  # 确保当前路径是文件夹
            rdf_file = os.path.join(folder_path, 'rdf_Li_H2O.xvg')
            cn_file = os.path.join(folder_path, 'cn_Li_H2O.xvg')
